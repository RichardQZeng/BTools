...
def workLines(dict_segment):
    ...
    destination = [shapes for shapes in geopandas.GeoDataFrame(geometry=[destination_point], crs=shapefile_proj).geometry]
    ...
    try:
        ...
        mcp_source=skimage.graph.MCP_Geometric(clip_CostR,sampling=(cellSizex, cellSizey))
        ...
        mcp_Dest = skimage.graph.MCP_Geometric(clip_CostR, sampling=(cellSizex, cellSizey))
        ...
        Out_polygon=rasterio.features.shapes(cleanRaster,mask=mask,transform=out_transform1)
...
def lineprepare(callback,**args):
    ...
    list_dict_segment_all=SpliteLine2(**args)
...
if __name__ == '__main__':
    ...
    list_dict_segment_all=lineprepare(print, **args.input)


    out_footprint=args.input['out_footprint']
    ...
    #     footprint_list.append(workLines(row))

    # Old multiprocessing
    pool = multiprocessing.Pool(processes=multiprocessing.cpu_count())
    footprint_list=pool.map(workLines,list_dict_segment_all)
    ...
    # try:
        ...
        #     step = 0
        #
        #     for result in pool.imap_unordered(workLines,list_dict_segment_all):
        #         results = geopandas.GeoDataFrame(pandas.concat([results, result], ignore_index=False))
        #         # footprint_list.append(result)
    ...
    #dissovled polygon group by column 'Fr_Orig_ln'
    dissovled_results=results.dissolve('Fr_Orig_ln')

    dissovled_results.to_file(out_footprint)

    print('Finishing footprint processing @ {} (or in {} second)'.format(time.strftime("%a, %d %b %Y %H:%M:%S"
                                                        ,time.localtime()),time.time() - start_time))
